    ...
    value: function bindTriggers(officer, counter, items) {
      officer.querySelector('.plus').addEventListener('click', function () {
        if (counter !== items.length - 2) {
          // якщо зараз каунтер ще не дійшов до передостаннього елементу
          items[counter].classList.add('animated', 'fadeIn');
          items[counter].style.display = 'flex';
          counter++;
        } else {
          // коли показується передстання карточка, остання карточка видаляється
          items[counter].classList.add('animated', 'fadeIn');
          items[counter].style.display = 'flex';
...
function () {
  ...
  _createClass(Form, [{
    ...
    value: function clearInputs() {
      // 3. Очистка інпутів після відправки форми 
    ...
    value: function checkMailInputs() {
      ...
      mailInputs.forEach(function (inp) {
        inp.addEventListener('keypress', function (e) {
          if (e.key.match(/[^a-z 0-9 @ \.]/ig)) {
            // якщо не латиниця, цифри, @ чи ., то не вводити символ
            e.preventDefault();
          }
        });
      });
